...
//이 값은 정수 또는 long 형태일 수 있으며, `System.out.println()`과 같은 방식으로 직접 출력할 수 있습니다.

//반면 `max()`는 스트림에서 최대값을 찾아 `OptionalInt` 형태로 반환합니다. 
//스트림이 비어있을 경우 최대값이 없을 수 있기 때문입니다. 
//따라서 `max()`의 결과를 출력하려면 `isPresent()`로 값이 존재하는지 확인한 후 `getAsInt()`와 같은 메서드를 사용하여 `OptionalInt`를 정수로 변환해야 합니다. 
